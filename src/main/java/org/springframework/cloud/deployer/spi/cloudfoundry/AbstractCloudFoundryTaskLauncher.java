	...
	private Mono<TaskStatus> getStatus(String id) {
		...
		return requestGetTask(id)
			...
			.otherwise(isNotFoundError(), t -> {
				logger.info("Task for id={} does not exist", id);
				return Mono.just(new TaskStatus(id, LaunchState.unknown, null));
			})
			.timeout(Duration.ofMillis(requestTimeout))
			.doOnError(e -> logger.error(String.format("Error getting status for id=%s within %s millseconds, Retrying operation", id, requestTimeout), e))
			...
			.doOnSuccess(r -> logger.info("Successfully retried getStatus operation for id={}", id))
			.doOnError(e -> logger.error(String.format("Retry operation on getStatus failed for id=%s", id), e))
